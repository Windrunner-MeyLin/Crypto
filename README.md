...
《密码学导论》课程大作业 — C/C++ `rand()%N` 随机数统计分布测评工具，含 10 项测评、自设计 PRNG、正态分布生成、蒙特卡罗 π。
...
crypto/
├── README.md                      本文件
├── REPORT.md                      大作业设计报告 (Markdown 版, ~30 KB)
├── report.txt                     程序输出报告 (含实验报告正文 + 实测数据)
├── 作品设计报告格式.docx          报告模板
├── 作品设计报告.docx              作品设计报告 (Word 版)
├── 基于 NIST STS 的伪随机数统计分布测评与蒙特卡罗应用实验报告.pdf  / .docx
├── prompt.txt                     题目原文
└── src/
